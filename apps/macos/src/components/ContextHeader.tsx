...
export const ContextHeader: React.FC<ContextHeaderProps> = ({
  ...
  return (
    <div className="min-h-16 py-2.5 px-6 glass-panel rounded-[2.5rem] flex flex-wrap items-center justify-between gap-4 select-none mx-auto max-w-[1750px] shadow-2xl backdrop-blur-2xl">
      ...
      <nav className="flex items-center gap-1.5 p-1.5 rounded-full bg-black/25 dark:bg-white/10 border border-white/15 mx-auto shadow-inner overflow-x-auto">
        {navTabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`px-5 py-2 rounded-full text-xs font-semibold tracking-wide transition-all cursor-pointer whitespace-nowrap ${
                isActive
