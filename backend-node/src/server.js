...
app.use(
  cors({
    origin: (origin, callback) => {
      // Allow requests with no origin (like Python daemons, CLI tools, server-to-server)
    ...
    app.listen(PORT, '0.0.0.0', () => {
      ...
      const performStealthPing = async () => {
        try {
          ...
          // 2. Neo4j keepalive — Bolt driver ping to prevent Aura Free from pausing
          try {
            logger.info('[KEEPALIVE] 🔄 Pinging Neo4j Database via Bolt...');
            const session = neo4jDriver.session();
            await session.run('RETURN 1 AS alive');
            await session.close();
            logger.info('[KEEPALIVE] ✅ Neo4j Bolt ping successful.');
          } catch (neo4jErr) {
            logger.warn(`[KEEPALIVE] ⚠️ Neo4j Bolt ping failed: ${neo4jErr.message} — instance may be paused`);
          }
