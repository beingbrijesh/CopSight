...
1. **Single-File API Layer (`APIService.swift`)**: Mirroring the web frontend's [`frontend/src/lib/api.ts`](../../frontend/src/lib/api.ts), all networking, generic async/await requests, JWT Bearer injection, and namespaced endpoints reside in a single file `copsight-macos/Sources/CopSightAI/Services/APIService.swift`.
