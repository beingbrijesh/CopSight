...
from forensixd.core.models import ExtractionLevel, Platform
...
def acquire(output_dir, level, ufdr_config):
    ...
    with Progress(SpinnerColumn(), TextColumn("{task.description}"), transient=True) as p:
        ...
        devices = detector.scan(); p.remove_task(t)
    if not devices:
        console.print("[red]No devices found.[/red]"); sys.exit(1)
    device = devices[0]
    console.print(f"[green]Found:[/green] {device.platform.value} — {device.device_id}")

    # Step 2: authorization
    try: case = AuthorizationManager.capture_interactively(device)
    except (AuthorizationError, ForensixdError) as e:
        console.print(f"[red]Authorization failed:[/red] {e}"); sys.exit(1)

    # Step 3: extractor
    try: extractor_cls = ExtractorRegistry.get(device.platform)
    except ForensixdError as e:
        console.print(f"[red]No extractor:[/red] {e}"); sys.exit(1)
    extractor = extractor_cls()
    if not extractor.is_available():
        console.print("[red]Required libraries not installed.[/red]"); sys.exit(1)
    ...
    for a in artifacts: w.append_artifact(a)
    w.finalize()
    ...
    tbl.add_column("Output"); tbl.add_column("Value")
...
def verify(session_dir):
    ...
    if not files: console.print("[red]No audit log found.[/red]"); sys.exit(1)
    ok_all = True
    for f in files:
