/**
 * System-Wide Audit & Diagnostic Logging Service for CopSight Web & Cloud Frontend.
 * Records user interactions, admin actions, API requests, and errors across the platform.
 */
...
class LoggerService {
  ...
  public initGlobalListeners(): void {
    ...
    this.info('SYSTEM', 'CopSight Web Enterprise Audit Logging Engine Initialized.');
  }

  private loadPersistedLogs(): void {
    try {
      const saved = localStorage.getItem('copsight_web_audit_trail');
      if (saved) {
  ...
  private persistLogs(): void {
    try {
      localStorage.setItem('copsight_web_audit_trail', JSON.stringify(this.logs.slice(-250)));
    } catch {
      // Ignore quota limits
    }
  }
  ...
  public clearLogs(): void {
    this.logs = [];
    localStorage.removeItem('copsight_web_audit_trail');
    this.notifyLogListeners();
  ...
  public exportLogsJson(): void {
    ...
    try {
      ...
      downloadAnchor.setAttribute('download', `copsight_web_audit_${new Date().toISOString().replace(/[:.]/g, '-')}.json`);
      ...
      this.triggerToast({
        ...
        message: `Saved JSON audit log dossier (${this.logs.length} entries).`,
        timestamp: Date.now(),
      });
    } catch {
